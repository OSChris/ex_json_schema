defmodule ExJsonSchema.Validator do
  ...
  defp validate_aspect(root, _, {"allOf", all_of}, data) do
    ...
    case Enum.empty?(invalid_indexes) do
      true -> []
      false ->
        [{%{key: "number_all_of", msg: "Expected all of the schemata to match, but the schemata at the following indexes did not: #{Enum.join(invalid_indexes, ", ")}."}, []}]
    end
  end
  ...
  defp validate_aspect(root, _, {"anyOf", any_of}, data) do
    case Enum.any?(any_of, &valid?(root, &1, data)) do
      true -> []
      false -> [{%{key: "number_any_of", msg: "Expected any of the schemata to match but none did."}, []}]
    end
  end
  ...
  defp validate_aspect(root, _, {"oneOf", one_of}, data) do
    ...
    case Enum.empty?(valid_indexes) do
      true -> [{%{key: "number_one_of", msg: "Expected exactly one of the schemata to match, but none of them did."}, []}]
      false -> if Enum.count(valid_indexes) == 1 do
          []
        else
          [{%{key: "number_one_of", msg: "Expected exactly one of the schemata to match, but the schemata at the following indexes did: #{Enum.join(valid_indexes, ", ")}."}, []}]
        end
    end
  end
  ...
  defp validate_aspect(root, _, {"not", not_schema}, data) do
    case valid?(root, not_schema, data) do
      true -> [{%{key: "number_not", msg: "Expected schema not to match but it did."}, []}]
      false -> []
    end
  end

  defp validate_aspect(first, second, {"type", type}, data) do
  ...
  defp validate_aspect(_, _, {"minProperties", min_properties}, data) when is_map(data) do
    case Map.size(data) >= min_properties do
      true -> []
      false -> [{%{key: "array_min_properties", msg: "Expected a minimum of #{min_properties} properties but got #{Map.size(data)}"}, []}]
    end
  end
  ...
  defp validate_aspect(_, _, {"maxProperties", max_properties}, data) when is_map(data) do
    case Map.size(data) <= max_properties do
      true -> []
      false -> [{%{key: "array_max_properties", msg: "Expected a maximum of #{max_properties} properties but got #{Map.size(data)}"}, []}]
    end
  end
  ...
  defp validate_aspect(_, _, {"required", required}, data) do
    Enum.flat_map List.wrap(required), fn property ->
      case Map.has_key?(data, property) do
        true -> []
        false -> [{%{key: "required", property: property, msg: "Required property #{property} was not present."}, []}]
      end
    end
  end
  ...
  defp validate_aspect(_, _, {"minItems", min_items}, items) when is_list(items) do
    case (count = Enum.count(items)) >= min_items do
      true -> []
      false -> [{%{key: "array_min_items", msg: "Expected a minimum of #{min_items} items but got #{count}."}, []}]
    end
  end
  ...
  defp validate_aspect(_, _, {"maxItems", max_items}, items) when is_list(items) do
    case (count = Enum.count(items)) <= max_items do
      true -> []
      false -> [{%{key: "array_max_items", msg: "Expected a maximum of #{max_items} items but got #{count}."}, []}]
    end
  end
  ...
  defp validate_aspect(_, _, {"uniqueItems", true}, items) when is_list(items) do
    case Enum.uniq(items) == items do
      true -> []
      false -> [{%{key: "unique", msg: "Expected items to be unique but they were not."}, []}]
    end
  end
  ...
  defp validate_aspect(_, _, {"enum", enum}, data) do
    case Enum.any?(enum, &(&1 === data)) do
      true -> []
      false -> [{%{key: "enum", msg: "Value #{inspect(data)} is not allowed in enum."}, []}]
    end
  end
  ...
  defp validate_aspect(_, schema, {"minimum", minimum}, data) when is_number(data) do
    ...
    case fun.(data, minimum) do
      true -> []
      false -> [{%{key: "#{if exclusive?, do: "number_gt", else: "number_gte"}", msg: "Expected the value to be #{if exclusive?, do: "<", else: "<="} #{minimum}" }, []}]
    end
  end
  ...
  defp validate_aspect(_, schema, {"maximum", maximum}, data) when is_number(data) do
    ...
    case fun.(data, maximum) do
      true -> []
      false -> [{%{key: "#{if exclusive?, do: "number_lt", else: "number_lte"}", msg: "Expected the value to be #{if exclusive?, do: ">", else: ">="} #{maximum}" }, []}]
    end
  end
  ...
  defp validate_aspect(_, _, {"multipleOf", multiple_of}, data) when is_number(data) do
    ...
    case Float.floor(factor) == factor do
      true -> []
      false -> [{%{key: "multiple_of", msg: "Expected value to be a multiple of #{multiple_of} but got #{data}."}, []}]
    end
  end
  ...
  defp validate_aspect(_, _, {"minLength", min_length}, data) when is_binary(data) do
    case (length = String.length(data)) >= min_length do
      true -> []
      false -> [{%{key: "string_lte", msg: "Expected value to have a minimum length of #{min_length} but was #{length}."}, []}]
    end
  end
  ...
  defp validate_aspect(_, _, {"maxLength", max_length}, data) when is_binary(data) do
    case (length = String.length(data)) <= max_length do
      true -> []
      false -> [{%{key: "string_gte", msg: "Expected value to have a maximum length of #{max_length} but was #{length}."}, []}]
    end
  end
  ...
  defp validate_aspect(_, _, {"pattern", pattern}, data) when is_binary(data) do
    case pattern |> Regex.compile! |> Regex.match?(data) do
      true -> []
      false -> [{%{key: "pattern", msg: "String #{inspect(data)} does not match pattern #{inspect(pattern)}."}, []}]
    end
  end
